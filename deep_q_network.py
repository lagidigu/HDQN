...
class dqn:
    ...
    def store(self, state, action, reward, next_state, terminal):
        ...
        #experience = np.hstack((state, action, reward, next_state, terminal))
        experience = [state[0], state[1], action, reward, next_state[0], next_state[1], terminal]
    ...
    def train(self):
        ...
        _, __ = self.session.run([self.optimizer, self.loss], feed_dict={self.input_state : batch[:, :self.num_features],
                                                                         ...
                                                                         self.input_state_next : batch[:, -self.num_features:]})
